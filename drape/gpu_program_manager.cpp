...
public:
  ...
  {
    std::map<int, gpu::ProgramInfo>::const_iterator it = m_mapping.find(program);
    ASSERT(it != m_mapping.end(), ());
    return it->second;
  }

private:
  std::map<int, gpu::ProgramInfo> m_mapping;
};
...
ref_ptr<Shader> GpuProgramManager::GetShader(int index, std::string const & source, Shader::Type t)
{
