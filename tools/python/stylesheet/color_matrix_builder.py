...
class MatrixBuilder:
    ...
    def filter_fn(self, element):
        ...
        for key in self.color_attrs(element):
            if self.has_color_with_coords(element, key):
                return True
        return False


    def color_attrs(self, element):
        return filter(lambda x: x.endswith("color"), dir(element))
    ...
    def key_maker(self, element, keys, zoom):
        ...
        for color_key in self.color_attrs(element):
