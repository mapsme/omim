...
public:
  ...
  void operator()(FeatureType & f, uint32_t)
  {
    ...
    if (f.GetGeomType() == feature::GeomType::Line)
    {
      ...
      f.ForEachPoint(
          [&len, &firstPoint, &lastPoint](const m2::PointD & pt) {
            if (firstPoint)
              firstPoint = false;
            else
    ...
    {
      f.ForEachTriangle(
          [&area](m2::PointD const & p1, m2::PointD const & p2, m2::PointD const & p3) {
...
std::string GetKey(CountType t) { return strings::to_string(t.m_val); }

std::string GetKey(ClassifType t) { return classif().GetFullObjectName(t.m_val); }

std::string GetKey(AreaType t) { return strings::to_string(arrAreas[t.m_val]); }
...
void PrintTop(char const * prefix, TSet const & theSet)
{
  ...
  vector<pair<typename TSet::key_type, typename TSet::mapped_type>> vec(theSet.begin(),
                                                                        theSet.end());
