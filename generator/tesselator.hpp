...
class TrianglesInfo
{
  ...
  class ListInfo
  {
    ...
    struct HashPair
    {
      size_t operator()(std::pair<T1, T2> const & p) const { return base::Hash(p.first, p.second); }
    };
  ...
  public:
    ...
    ListInfo(size_t count) { m_triangles.reserve(count); }
  ...
  private:
    ...
    void MakeTrianglesChainImpl(PointsInfo const & points, TIterator start,
