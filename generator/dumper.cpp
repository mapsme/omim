...
void DumpPrefixes(string const & fPath)
{
  ...
  feature::ForEachFromDat(fPath, doClass);
  for (TokensContainerT::iterator it = doClass.m_stats.begin(); it != doClass.m_stats.end(); ++it)
  {
...
void DumpFeatureNames(string const & fPath, string const & lang)
{
  ...
  feature::ForEachFromDat(fPath, [&](FeatureType & f, uint32_t) { f.ForEachName(printName); });
}
}  // namespace feature
