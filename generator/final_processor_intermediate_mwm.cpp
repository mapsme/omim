...
{
  ThreadPool pool(m_threadsCount);
  std::vector<std::future<std::vector<indexer::HierarchyEntry>>> futures;
  ...
  std::vector<indexer::HierarchyEntry> allLines;
  for (auto & f : futures)
  {
...
void ComplexFinalProcessor::WriteLines(std::vector<indexer::HierarchyEntry> const & lines)
{
  std::ofstream stream;
