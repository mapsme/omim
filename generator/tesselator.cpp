...
void TrianglesInfo::ListInfo::AddNeighbour(int p1, int p2, int trg)
{
  ...
  std::pair<TNeighbours::iterator, bool> ret =
      m_neighbors.insert(std::make_pair(std::make_pair(p1, p2), trg));
...
TrianglesInfo::ListInfo::TIterator TrianglesInfo::ListInfo::FindStartTriangle(
    PointsInfo const & points) const
{
...
std::pair<int, int> CommonEdge(Triangle const & to, Triangle const & from)
{
  for (int i = 0; i < 3; ++i)
  {
    for (int j = 0; j < 3; ++j)
    {
      if (to.m_p[i] == from.m_p[base::NextModN(j, 3)] &&
          to.m_p[base::NextModN(i, 3)] == from.m_p[j])
...
/// - nb[0] - by 1->2 edge;
/// - nb[1] - by 2->0 edge;
void TrianglesInfo::ListInfo::GetNeighbors(Triangle const & trg, Triangle const & from,
                                           int * nb) const
{
...
uint64_t TrianglesInfo::ListInfo::CalcDelta(PointsInfo const & points, Triangle const & from,
                                            Triangle const & to) const
{
...
void TrianglesInfo::ListInfo::MakeTrianglesChain(PointsInfo const & points, TIterator start,
...
void TrianglesInfo::Add(int p0, int p1, int p2) { m_triangles.back().Add(p0, p1, p2); }
