...
UNIT_TEST(Source_To_Element_create_from_xml_test)
{
  ...
  ProcessOsmElementsFromXML(reader, [&elements](OsmElement * e) { elements.push_back(*e); });
...
UNIT_TEST(Source_To_Element_create_from_o5m_test)
{
  ...
  std::vector<OsmElement> elements;
  ProcessOsmElementsFromO5M(reader, [&elements](OsmElement * e) { elements.push_back(*e); });
...
UNIT_TEST(Source_To_Element_check_equivalence)
{
  ...
  ProcessOsmElementsFromXML(readerXML,
                            [&elementsXML](OsmElement * e) { elementsXML.push_back(*e); });
  ...
  ProcessOsmElementsFromO5M(readerO5M,
                            [&elementsO5M](OsmElement * e) { elementsO5M.push_back(*e); });
