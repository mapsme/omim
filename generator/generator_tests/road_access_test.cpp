...
namespace
{
std::string const kTestDir = "road_access_generation_test";
std::string const kTestMwm = "test";
std::string const kRoadAccessFilename = "road_access_in_osm_ids.csv";
...
void LoadRoadAccess(std::string const & mwmFilePath, RoadAccess & roadAccess)
{
...
void TestRoadAccess(std::string const & roadAccessContent, std::string const & mappingContent)
{
  Platform & platform = GetPlatform();
  std::string const writableDir = platform.WritableDir();

  // Building empty mwm.
  LocalCountryFile country(my::JoinPath(writableDir, kTestDir), CountryFile(kTestMwm),
                           0 /* version */);
  ScopedDir const scopedDir(kTestDir);
  std::string const mwmRelativePath = my::JoinPath(kTestDir, kTestMwm + DATA_FILE_EXTENSION);
  ScopedFile const scopedMwm(mwmRelativePath);
  BuildEmptyMwm(country);

  // Creating a file with road access.
  std::string const roadAccessRelativePath = my::JoinPath(kTestDir, kRoadAccessFilename);
  ScopedFile const roadAccessFile(roadAccessRelativePath, roadAccessContent);

  // Creating osm ids to feature ids mapping.
  std::string const mappingRelativePath = my::JoinPath(kTestDir, kOsmIdsToFeatureIdsName);
  ScopedFile const mappingFile(mappingRelativePath);
  std::string const mappingFullPath = mappingFile.GetFullPath();
  ReEncodeOsmIdsToFeatureIdsMapping(mappingContent, mappingFullPath);

  // Adding road access section to mwm.
  std::string const roadAccessFullPath = my::JoinPath(writableDir, roadAccessRelativePath);
  std::string const mwmFullPath = my::JoinPath(writableDir, mwmRelativePath);
...
{
  std::string const roadAccessContent = "";
  std::string const osmIdsToFeatureIdsContent = "";
  TestRoadAccess(roadAccessContent, osmIdsToFeatureIdsContent);
}

UNIT_TEST(RoadAccess_AccessPrivate)
{
  std::string const roadAccessContent = R"(access=private 0)";
  std::string const osmIdsToFeatureIdsContent = R"(0, 0,)";
  TestRoadAccess(roadAccessContent, osmIdsToFeatureIdsContent);
}

UNIT_TEST(RoadAccess_Access_And_Barriers)
{
  std::string const roadAccessContent = R"(access=private 10
                                     access=private 20
                                     barrier=gate 30
                                     barrier=gate 40)";
  std::string const osmIdsToFeatureIdsContent = R"(10, 1,
                                             20, 2,
                                             30, 3,
                                             40, 4,)";
  TestRoadAccess(roadAccessContent, osmIdsToFeatureIdsContent);
