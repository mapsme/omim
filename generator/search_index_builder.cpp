...
void BuildAddressTable(FilesContainerR & container, Writer & writer, uint32_t threadsCount)
{
  ...
  {
    // Code corresponds HouseToStreetTable decoding.
    ...
    // First street id in the block encoded as VarInt<uint32_t>, all other street ids in the block
    // encoded as VarInt<int32_t> delta from previous id.
    ...
    LOG(LINFO, ("Address: Building -> Street (opt, all)", houseToStreetCount));
  }
