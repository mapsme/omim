...
int GeneratorToolMain(int argc, char ** argv)
{
  ...
  if (FLAGS_generate_features || FLAGS_make_coasts || FLAGS_generate_world)
  {
    ...
    if (FLAGS_generate_features)
    {
      ...
      auto const translatorType =
          FLAGS_no_ads ? TranslatorType::Country : TranslatorType::CountryWithAds;
      translators.Append(
          CreateTranslator(translatorType, emitter, cacheLoader.GetCache(), genInfo));
    }
    ...
    if (FLAGS_generate_world)
    {
      ...
      auto const translatorType =
          FLAGS_no_ads ? TranslatorType::World : TranslatorType::WorldWithAds;
      translators.Append(
          CreateTranslator(translatorType, emitter, cacheLoader.GetCache(), genInfo));
    }
    ...
    if (FLAGS_make_coasts)
    {
      ...
      translators.Append(
          CreateTranslator(TranslatorType::Coastline, emitter, cacheLoader.GetCache()));
    }
  ...
  if (!FLAGS_geo_objects_key_value.empty())
  {
    if (!geo_objects::GenerateGeoObjects(
            FLAGS_regions_index, FLAGS_regions_key_value, FLAGS_geo_objects_features,
            FLAGS_ids_without_addresses, FLAGS_geo_objects_key_value,
            FLAGS_allow_addressless_for_countries, FLAGS_verbose, threadsCount))
      return EXIT_FAILURE;
  }
