...
void LogParser::ParseUserTracks(string const & logFile, UserToTrack & userToTrack) const
{
  ...
  std::regex const base_regex(".*(DataV0|CurrentData)\\s+aloha_id\\s*:\\s*(\\S+)\\s+.*\\|(\\w+)\\|");
  ...
  LOG(LINFO, ("Tracks parsing finished, elapsed:", timer.ElapsedSeconds(), "seconds, lines:", linesCount, ", points", pointsCount));
...
void LogParser::SplitIntoMwms(UserToTrack const & userToTrack, MwmToTracks & mwmToTracks) const
{
  ...
  LOG(LINFO, ("Data was splitted into", mwmToTracks.size(), "mwms, elapsed:", timer.ElapsedSeconds(), "seconds"));
}
}  // namespace tracking
