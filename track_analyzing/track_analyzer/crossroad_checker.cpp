...
IsCrossroadChecker::Type IsCrossroadChecker::operator()(Segment const & current, Segment const & next) const
{
  ...
  auto const currentSegmentHwType = m_geometry.GetRoad(currentSegmentFeatureId, true /* isOutgoing */).GetHighwayType();
  auto const nextSegmentFeatureId = next.GetFeatureId();
  auto const nextSegmentHwType = m_geometry.GetRoad(nextSegmentFeatureId, true /* isOutgoing */).GetHighwayType();
