...
double DistanceToSegment(Segment const & segment, m2::PointD const & point, IndexGraph & indexGraph)
{
  return DistanceToSegment(
      mercator::FromLatLon(indexGraph.GetGeometry().GetPoint(segment.GetRoadPoint(false), true /* isOutgoing */)),
      mercator::FromLatLon(indexGraph.GetGeometry().GetPoint(segment.GetRoadPoint(true), true /* isOutgoing */)), point);
}
