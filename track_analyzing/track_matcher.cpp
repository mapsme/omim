...
void TrackMatcher::Step::FillCandidatesWithNearbySegments(Index const & index, routing::IVehicleModel const & vehicleModel, routing::NumMwmId mwmId)
{
  index.ForEachInRect(
...
void TrackMatcher::Step::ChooseSegment(Step const & nextStep, routing::IndexGraph & indexGraph)
{
  ...
  CHECK_LESS(minDistance, numeric_limits<double>::max(), ("Can't find previous segment for", nextStep.m_segment));
}
