...
- (void)setFirstButton:(id<MWMActionBarSharedData>)data
{
  ...
  BOOL const isNavigationActive =
      [MWMNavigationDashboardManager manager].state == MWMNavigationDashboardStateNavigation;
  if (!self.isAreaNotDownloaded && !isNavigationActive && !isRoutePoint)
...
- (void)setSecondButton:(id<MWMActionBarSharedData>)data
{
  ...
  BOOL const isRoutePoint = [data isRoutePoint];
  if (isRoutePoint)
    buttons.push_back(EButton::RouteRemoveStop);
...
- (void)addBarButtons:(id<MWMActionBarSharedData>)data
{
  BOOL const isPartner = [data isPartner] && [data sponsoredURL] != nil;
