  ...
  else if (indexPath.section == m_bookmarkSection)
  {
    if (cat)
    {
      ...
      if (bm)
      {
        ...
        f.ShowBookmark({static_cast<int>(indexPath.row), static_cast<int>(m_categoryIndex)});
        [self.navigationController popToRootViewControllerAnimated:YES];
      }
    }
  }
  ...
  if (indexPath.section == m_trackSection || indexPath.section == m_bookmarkSection)
  {
    ...
    if (cat)
    {
      if (editingStyle == UITableViewCellEditingStyleDelete)
      {
        ...
        {
          BookmarkAndCategory bookmarkAndCategory{static_cast<int>(indexPath.row),
                                                  static_cast<int>(m_categoryIndex)};
