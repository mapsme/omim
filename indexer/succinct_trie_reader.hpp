...
public:
  ...
  {
    return my::make_unique<SuccinctTrieIterator>(m_reader, m_common, m_nodeBitPosition);
  }
  ...
  std::unique_ptr<SuccinctTrieIterator> GoToEdge(size_t i) const
  {
    ...
    return my::make_unique<SuccinctTrieIterator>(m_reader, m_common, childBitPosition);
  }
    ...
    TReader const & reader, TValueReader valueReader = TValueReader())
{
  ...
  return my::make_unique<TIter>(common->GetReader(), common, 1 /* bitPosition */);
}
