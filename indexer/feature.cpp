...
{
  uint8_t header = Header() & HEADER_GEOTYPE_MASK;
  ...
  m_bTypesParsed = true;
  m_header = CalculateHeader(typesCount, header, m_params);
  m_bHeader2Parsed = true;
...
bool FeatureType::FromXml(editor::XMLFeature const & xml)
{
  ...
  xml.ForEachTag([this, &typesCount](string const & k, string const & v)
  {
    ...
    else
    {
      ...
      if (type == 0)
        type = cl.GetTypeByPathSafe({k});
      if (type == 0)
        type = cl.GetTypeByPathSafe({"amenity", k});

      if (type)
        m_types[typesCount++] = type;
      else
        LOG(LWARNING, ("Can't load type:", k, v));
    }
  });
  m_bMetadataParsed = true;
  m_bTypesParsed = true;
