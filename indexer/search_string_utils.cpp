...
public:
  ...
  bool MatchWithMisprints(DFA const & dfa) const
  {
    ...
    using DFAIt = strings::LevenshteinDFA::Iterator;
    ...
    while (!q.empty())
    {
      ...
      trieIt.ForEachMove([&q, &dfaIt](UniChar const & c, TrieIt const & curDfaIt) {
        auto curIt = dfaIt;
        curIt.Move(c);
        strings::DFAMove(curIt, curDfaIt.GetLabel());
        if (!curIt.Rejects())
          q.emplace(curDfaIt, curIt);
      });
    }
