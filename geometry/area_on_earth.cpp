...
long double AngleOnEarth(LatLon const & lla, LatLon const & llb, LatLon const & llc)
{
  ...
  m2::PointD vec1(pb.x - pa.x, pb.y - pa.y);
  m2::PointD vec2(pc.x - pb.x, pc.y - pb.y);
                        ...
                        double epsRad)
{
  ...
  a = a.Normalize();
  b = b.Normalize();
  double const oneLineCondition = fabs(m2::CrossProduct(a, b));
  return fabs(oneLineCondition) < epsRad;
}
...
double AreaOnEarth(std::vector<LatLon> const & latlons)
{
  ...
  long double const eps =
      anglesSum - static_cast<long double>(latlons.size() - 2) * math::pi;
...
double AreaOnEarthWithConvexHull(std::vector<m2::PointD> const & points)
{
  ...
  double constexpr kConvexHullEps = 1e-20;
...
{
  auto pointsCopy = area_on_sphere_details::SimplifyPolygon(points);
  ...
  if (pointsCopy.size() > 4)
  {
    ...
    double const area = area_on_sphere_details::AreaOnEarth(latlons);
    if (area > 0)
      return area;
  }
