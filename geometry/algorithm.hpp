    ...
    -> decltype(calc.GetResult())
{
  return impl::SelectImplementation(begin, end, std::forward<TCalculator>(calc),
                                    std::is_same<CalculatePointOnSurface,
                                            typename std::remove_reference<TCalculator>::type>());
}
...
auto ApplyCalculator(TCollection && collection, TCalculator && calc)
    -> decltype(calc.GetResult())
{
  return ApplyCalculator(begin(collection), end(collection), std::forward<TCalculator>(calc));
}
}  // namespace m2
