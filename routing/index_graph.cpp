...
bool IndexGraph::IsJointOrEnd(Segment const & segment, bool fromStart)
{
  ...
  uint32_t const pointsNumber = GetGeometry().GetRoad(segment.GetFeatureId(), true /* isOutgoing */).GetPointsCount();
                                            ...
                                            std::optional<RouteWeight const> const & prevWeight)
{
  ...
  auto const weight = RouteWeight(m_estimator->CalcSegmentWeight(segment, road, purpose, isOutgoing));
  auto const & penalties =
      GetPenalties(purpose, isOutgoing ? from : to, isOutgoing ? to : from, prevWeight, isOutgoing);
