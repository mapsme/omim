                          ...
                          WorldGraph & graph)
{
  ...
  for (size_t i = 0; i < segments.size(); ++i)
  {
    ...
    bool const oneWay = graph.IsOneWay(segment.GetMwmId(), segment.GetFeatureId(), true);
    auto const & frontJunction = graph.GetJunction(segment, true /* front */, true);
    auto const & backJunction = graph.GetJunction(segment, false /* front */, true);
