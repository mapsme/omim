...
void SpeedCameraManager::OnLocationPositionChanged(location::GpsInfo const & info)
{
  ...
  if (!m_cachedSpeedCameras.empty() && distFromCurrentPosAndClosestCam < 0)
  {
    ...
    if (NeedUpdateClosestCamera(passedDistanceMeters, info.m_speedMpS, closestSpeedCam))
    {
...
{
  CHECK(m_closestCamera.IsValid(), ("Try to show invalid speed cam."));
...
bool SpeedCameraManager::NeedUpdateClosestCamera(double passedDistanceMeters, double speedMpS,
                                                 SpeedCameraOnRoute const & camera)
{
  auto const distToNewCamera = camera.m_distFromBeginMeters - passedDistanceMeters;
  if (m_closestCamera.IsValid())
  {
    auto const distToOldCamera = m_closestCamera.m_distFromBeginMeters - passedDistanceMeters;

    // If we passed the previous camera and the next is close enough to work with it.
    return distToOldCamera < 0 && IsCameraCloseEnough(distToNewCamera);
  }

  return IsCameraCloseEnough(distToNewCamera);
}
