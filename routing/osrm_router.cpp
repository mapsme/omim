...
public:
  ...
  void CalculateOffset(OsrmMappingTypes::FtSeg const & seg, m2::PointD const & segPt, NodeID & nodeId, int & offset, bool forward) const
  {
    ...
    if (m_dataFacade.GetOutDegree(nodeId))
    {
      offset = 0;
      return;
    }
    ...
    double ratio = distance / fullDist;

    auto beginEdge = m_dataFacade.BeginEdges(nodeId);
    EdgeWeight minWeight = m_dataFacade.GetEdgeData(beginEdge).distance;
    for (EdgeID i = ++beginEdge; i < m_dataFacade.EndEdges(nodeId); ++i)
      minWeight = min(m_dataFacade.GetEdgeData(i).distance, minWeight);
