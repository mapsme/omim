...
void IndexRoadGraph::GetRouteEdges(EdgeVector & edges) const
{
  ...
  for (Segment const & segment : m_segments)
  {
    auto const & junctionFrom =
        m_starter.GetJunction(segment, false /* front */, true /* isOutgoing */).ToPointWithAltitude();
    auto const & junctionTo =
        m_starter.GetJunction(segment, true /* front */, true /* isOutgoing */).ToPointWithAltitude();
