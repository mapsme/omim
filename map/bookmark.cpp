...
void BookmarkCategory::DeleteTrack(size_t index)
{
  ...
  SetDirty();
  ASSERT_LESS(index, m_tracks.size(), ());
  m_tracks.erase(std::next(m_tracks.begin(), index));
  ReleaseController();
}
...
namespace
{
  ...
  class KMLParser
  {
    ...
    map<std::string, dp::Color> m_styleUrl2Color;
    map<std::string, std::string> m_mapStyle2Style;
  ...
  public:
    ...
    bool IsValidAttribute(std::string const & type, std::string const & value, std::string const & attrInLowerCase) const
    {
...
void BookmarkCategory::SaveToKML(std::ostream & s)
{
       ...
       count < GetUserPointCount(); ++count, --i)
  {
    ...
    if (timeStamp != my::INVALID_TIME_STAMP)
    {
      ...
      ASSERT_EQUAL(strTimeStamp.size(), 20, ("We always std::generate fixed length UTC-format timestamp"));
