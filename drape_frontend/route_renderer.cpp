...
void RouteRenderer::RenderRoute(ScreenBase const & screen, ref_ptr<ThreadsCommutator> commutator,
                                ref_ptr<dp::GpuProgramManager> mng, dp::UniformValuesStorage const & commonUniforms)
{
  ...
  if (zoom >= kArrowAppearingZoomLevel && !m_routeData->m_sourceTurns.empty())
  {
    ...
    double const halfTextureWidth = halfWidth * kArrowHeightFactor;
    double const glbHalfTextureWidth = halfTextureWidth * screen.GetScale();
    ...
    else if (!AreEqualArrowBorders(newArrowBorders, m_arrowBorders))
    {
      ...
      commutator->PostMessage(ThreadsCommutator::ResourceUploadThread,
                              make_unique_dp<CacheRouteArrowsMessage>(m_routeData->m_routeIndex, m_arrowBorders),
    ...
    if (m_routeArrows != nullptr)
    {
      ...
      dp::UniformValuesStorage uniforms = commonUniforms;
      uniforms.SetFloatValue("u_arrowHalfWidth", halfTextureWidth);
