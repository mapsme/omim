...
void RouteShape::CacheRoute(ref_ptr<dp::TextureManager> textures, RouteData & routeData)
{
  ...
  if (!routeData.m_segment->m_traffic.empty())
  {
  ...
  else if (!routeData.m_segment->m_subwayColoring.empty())
  {
    segmentsColors.reserve(routeData.m_segment->m_subwayColoring.size());
    for (auto subwayColor : routeData.m_segment->m_subwayColoring)
    {
