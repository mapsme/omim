...
void ForEachShader(dp::ApiVersion apiVersion, std::string const & defines,
...
UNIT_TEST(CompileShaders_Test)
{
  ...
  for (auto const & compiler : compilers)
  {
    ...
    string defines = "";
    ForEachShader(compiler.m_apiVersion, defines, gpu::GetVertexShaders(compiler.m_apiVersion),
                  compilerPath, [](QProcess const &) {}, argsPrepareFn, successComparator, ss);
    shaderType = "-f";
    ForEachShader(compiler.m_apiVersion, defines, gpu::GetFragmentShaders(compiler.m_apiVersion),
                  compilerPath, [](QProcess const &) {}, argsPrepareFn, successComparator, ss);
...
UNIT_TEST(MALI_CompileShaders_Test)
{
  ...
  auto successComparator = [](QString const & output) {
  ...
  for (auto const & compiler : compilers)
  {
    for (auto const & set : compiler.m_driverSets)
    {
      for (auto const & version : set.m_releases)
      {
        ...
        ForEachShader(compiler.m_apiVersion, defines, gpu::GetVertexShaders(compiler.m_apiVersion),
                      compilerPath, procPrepare, argForming, successComparator, ss);
        shaderType = "-f";
        ForEachShader(compiler.m_apiVersion, defines,
                      gpu::GetFragmentShaders(compiler.m_apiVersion), compilerPath, procPrepare,
                      argForming, successComparator, ss);
