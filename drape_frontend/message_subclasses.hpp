...
{
public:
  explicit AddTrafficSegmentsMessage(vector<pair<string, m2::PolylineD>> const & segments)
    : m_segments(segments)
  {}

  Type GetType() const override { return Message::AddTrafficSegments; }
  vector<pair<string, m2::PolylineD>> const & GetSegments() const { return m_segments; }

private:
  vector<pair<string, m2::PolylineD>> m_segments;
};
