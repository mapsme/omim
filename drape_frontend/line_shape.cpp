...
void LineShape::Prepare(ref_ptr<dp::TextureManager> textures) const
{
  ...
  if (m_params.m_pattern.empty())
  {
    ...
    if (m_isSimple)
    {
      ...
      auto builder = my::make_unique<SimpleSolidLineBuilder>(p, m_spline->GetPath().size(), lineWidth);
    ...
    else
    {
      ...
      commonParamsBuilder(p);
      auto builder = my::make_unique<SolidLineBuilder>(p, m_spline->GetPath().size());
  ...
  else
  {
    ...
    auto builder = my::make_unique<DashedLineBuilder>(p, m_spline->GetPath().size());
