...
location::EMyPositionMode MyPositionController::GetNewModeOnLocationUpdate()
{
  ...
  if (!m_isPositionAssigned)
    return m_desiredInitMode;

  if (m_mode == location::NotFollowNoPosition)
    return location::NotFollow;

  return m_isInRouting ? location::FollowAndRotate : location::Follow;
}
                                            ...
                                            ScreenBase const & screen)
{
  ...
  if (!m_notFollowAfterPending && previousPositionIsObsolete && !m_positionIsObsolete)
  {
    canChangeNotFollowMode = m_allowToFollowAfterObsoletePosition;
    m_allowToFollowAfterObsoletePosition = false;
  }

  auto const newMode = GetNewModeOnLocationUpdate();
  if (!m_isPositionAssigned)
  {
    ChangeMode(newMode);
          ...
          (m_mode == location::NotFollow && canChangeNotFollowMode))
  {
    ...
    {
      if (!m_hints.m_isFirstLaunch && (m_mode == location::Follow || m_mode == location::FollowAndRotate))
      {
