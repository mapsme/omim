...
Controller::RenderParams::RenderParams(graphics::EDensity density,
                                       TInvalidateFn const & invalidateFn,
                                       TPushAnimFn const & pushAnimFn,
                                       TGetModelViewFn const & getModelViewFn,
                                       graphics::GlyphCache * glyphCache,
                                       graphics::Screen * cacheScreen)
  : m_Density(density),
    m_InvalidateFn(invalidateFn),
    m_pushAnim(pushAnimFn),
    m_getModelViewFn(getModelViewFn),
...
ScreenBase const & Controller::GetModelView() const
{
  ASSERT(m_getModelViewFn, ());
  return m_getModelViewFn();
}
...
void Controller::SetRenderParams(RenderParams const & p)
{
  ...
  m_pushAnim = p.m_pushAnim;
  m_getModelViewFn = p.m_getModelViewFn;
...
void Controller::PushAnimTask(shared_ptr<anim::Task> const & task)
{
  if (m_pushAnim)
    m_pushAnim(task);
}
