...
Ruler::RulerFrame::RulerFrame(gui::Controller * controller,
                              frame_end_fn const  & fn, double depth)
  : m_textLengthInPx(0)
  , m_scale(0.0)
  , m_depth(depth)
  , m_callback(fn)
...
Ruler::RulerFrame::RulerFrame(Ruler::RulerFrame const & other, frame_end_fn const & fn)
  : m_dl(other.m_dl)
...
{
  Screen * cs = m_controller->GetCacheScreen();
  double const k = m_controller->GetVisualScale();
...
void Ruler::RulerFrame::Draw(OverlayRenderer * r, const math::Matrix<double, 3, 3> & m)
{
  ...
  double const yOffset = -(2 + TextOffsetFromRuler * m_controller->GetVisualScale());
...
void Ruler::RulerFrame::CreateAnim(double startAlfa, double endAlfa, double timeInterval, double timeOffset, bool isVisibleAtEnd)
{
  ...
  m_frameAnim.reset(new AlfaAnimationTask(startAlfa, endAlfa, timeInterval, timeOffset));
  m_frameAnim->AddCallback(anim::Task::EEnded, bind(&RulerFrame::AnimEnded, this, isVisibleAtEnd));
  m_controller->PushAnimTask(m_frameAnim);
}
...
{
  if (!m_mainFrame)
    m_mainFrame.reset(new RulerFrame(m_controller, bind(&Ruler::MainFrameAnimEnded, this, _1, _2), depth()));
...
void Ruler::update()
{
  ...
  ScreenBase const & screen = m_controller->GetModelView();
