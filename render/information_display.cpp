...
#include "render/compass_arrow.hpp"
#include "render/ruler.hpp"
#include "render/location_state.hpp"
#include "render/alfa_animation_task.hpp"
...
InformationDisplay::InformationDisplay(ActiveMapsBridge * mapsBridge,Animator * animator)
{
  m_fontDesc.m_color = Color(0x4D, 0x4D, 0x4D, 0xCC);

  InitRuler();
  InitCountryStatusDisplay(mapsBridge);
  InitCompassArrow();
  InitLocationState(animator);
  InitDebugLabel();
  InitCopyright();
...
void InformationDisplay::InitRuler()
{
  Ruler::Params p;
...
void InformationDisplay::InitCountryStatusDisplay(ActiveMapsBridge * mapsBridge)
{
  ...
  p.m_activeMaps = mapsBridge;

  m_countryStatusDisplay.reset(new CountryStatusDisplay(p));
}

void InformationDisplay::InitCopyright()
{
...
void InformationDisplay::OnCompassTapped()
{
  m_locationState->StopCompassFollowing();
  m_locationState->RotateOnNorth();
  m_controller->Invalidate();
}

void InformationDisplay::InitCompassArrow()
{
  ...
  p.m_pivot = m2::PointD(0, 0);
  p.m_handler = bind(&InformationDisplay::OnCompassTapped, this);

  m_compassArrow.reset(new CompassArrow(p));
  m_compassArrow->setIsVisible(false);
}

void InformationDisplay::InitLocationState(Animator * animator)
{
  rg::State::Params p;
  ...
  p.m_animator = animator;

  m_locationState.reset(new State(p));
}
...
void InformationDisplay::setController(gui::Controller * controller)
{
  ...
  shared_ptr<anim::Task> task(new AlfaAnimationTask(1.0, 0.0, 0.15, 3.0));
  ...
  m_controller->PushAnimTask(task);
}
...
void InformationDisplay::SetWidgetPivotsByDefault(int screenWidth, int screenHeight)
{
  ...
  double const vs = m_controller->GetVisualScale();
...
shared_ptr<rg::State> const & InformationDisplay::locationState() const
{
