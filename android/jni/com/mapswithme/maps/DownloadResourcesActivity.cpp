...
extern "C"
{
  ...
  Java_com_mapswithme_maps_DownloadResourcesActivity_nativeStartNextFileDownload(JNIEnv * env, jclass clazz, jobject listener)
  {
    ...
    TCallback onFinish(std::bind(&DownloadFileFinished, jni::make_global_ref(listener), std::placeholders::_1));
    TCallback onProgress(std::bind(&DownloadFileProgress, jni::make_global_ref(listener), std::placeholders::_1));

    g_currentRequest.reset(HttpRequest::PostJson(GetPlatform().ResourcesMetaServerUrl(), curFile.m_fileName,
                                                 std::bind(&DownloadURLListFinished, std::placeholders::_1, onFinish, onProgress)));
