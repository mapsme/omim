...
public:
  ...
  booking::AvailabilityParams Build(JNIEnv * env, jobject bookingFilterParams)
  {
    ...
    jlong jcheckin = env->GetLongField(bookingFilterParams, m_checkinMillisecId) / 1000;
    result.m_checkin = booking::AvailabilityParams::Clock::from_time_t(jcheckin);

    jlong jcheckout = env->GetLongField(bookingFilterParams, m_checkoutMillisecId) / 1000;
