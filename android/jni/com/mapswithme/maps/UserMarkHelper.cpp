...
void InjectMetadata(JNIEnv * env, jclass const clazz, jobject const mapObject, feature::Metadata const & metadata)
{
  ...
  for (auto const t : metadata.GetPresentTypes())
  {
    ...
    auto metaString = jni::ScopedLocalRef(t == feature::Metadata::FMD_WIKIPEDIA ?
...
pair<jintArray, jobjectArray> NativeMetadataToJavaMetadata(JNIEnv * env, Metadata const & metadata)
{
  ...
  for (size_t i = 0; i < metaTypes.size(); i++)
  {
    ...
    auto metaString = jni::ScopedLocalRef(
          type == Metadata::FMD_WIKIPEDIA ?
                  jni::ToJavaString(env, metadata.GetWikiURL()) :
...
jobject CreateBookmark(int categoryId, int bookmarkId, string const & typeName, Metadata const & metadata)
{
  ...
  static jmethodID const ctorId = env->GetMethodID(g_bookmarkClazz, "<init>",
                                                   "(IILjava/lang/String;)V");
  ASSERT (ctorId, (jni::DescribeException()));
  auto const name = jni::ScopedLocalRef(jni::ToJavaString(env, typeName));
...
jobject CreateMapObject(int mapObjectType, string const & name, double lat, double lon, string const & typeName, Metadata const & metadata)
{
  JNIEnv * env = jni::GetEnv();
  // Java signature :
  // public MapObject(@MapObjectType int mapObjectType, String name, double lat, double lon, String typeName)
  static jmethodID const ctorId = env->GetMethodID(g_mapObjectClazz, "<init>",
...
jobject CreateMapObject(UserMark const * userMark)
{
  ...
  switch (userMark->GetMarkType())
  {
    ...
    {
      mapObject = CreateMapObject(kPoi, info.GetPinName(), userMark->GetPivot().x, userMark->GetPivot().y, info.GetPinType(), metadata);
      break;
    }
    case UserMark::Type::SEARCH:
    {
      mapObject = CreateMapObject(kSearch, info.GetPinName(), userMark->GetPivot().x, userMark->GetPivot().y, info.GetPinType(), metadata);
      break;
    }
