...
namespace android
{
  ...
  void Platform::SendPushWooshTag(std::string const & tag, vector<std::string> const & values)
  {
    if (values.empty())
      return;
  ...
  void Platform::SendMarketingEvent(std::string const & tag, map<std::string, std::string> const & params)
  {
    JNIEnv * env = jni::GetEnv();
