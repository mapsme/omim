...
struct GpsTrackInfo
{
  ...
  GpsTrackInfo & operator=(GpsInfo const & info) { return operator=(GpsTrackInfo(info)); }
};
...
static inline bool IsLatValid(double lat) { return lat != 0. && lat < 90. && lat > -90.; }
static inline bool IsLonValid(double lon) { return lon != 0. && lon < 180. && lon > -180.; }
...
static inline double BearingToAngle(double a) { return AngleToBearing(a); }
...
public:
  ...
  struct SingleLaneInfoClient
  {
      ...
      : m_isRecommended(singleLaneInfo.m_isRecommended)
    {
      ...
      transform(lane.cbegin(), lane.cend(), m_lane.begin(),
